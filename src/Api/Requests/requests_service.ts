            ...
            if (invalidDomains.indexOf(domain) === -1) {
                if (domain.includes('.edu')) {
                    ...
                    if (college) {
                        ...
                        return ({
                            success: true,
                            message: 'We didnt recognise the email domain, We will send you the verification email once our team verifies the domain'
                        })
                    }
                }
            }
...
const getRequests = async (
    ...
    const requests = await db.collection(COL.Requests).aggregate([
        {
            $match: filter
        },
        {
    ...
    ]).skip(index * 10).limit(10).toArray();

    const totalRequests = await db.collection(COL.Requests)
...
const updateRequest = async (
    ...
    const request: any = await db.collection(COL.Requests)
    ...
    if (accepted) {
        ...
        if (request.type === 'College Addition') {
            ...
            await db.collection(COL.Colleges).updateOne(
                { _id: request.collegeId }, { $push: { stream: request.stream } }
            )
            ...
            await db.collection(COL.Colleges).updateOne(
                { _id: request.collegeId }, { $push: { domain : request.domain } }
            )
            ...
            await db.collection(COL.Users).updateOne(
                {_id:request.raisedBy},{$set: {'collegeInfo.verified': true, status: 'Onboarded'}})
        }
        ...
        if(request.type === "College Id Verification"){
            await db.collection(COL.Users).updateOne(
                {_id:request.raisedBy},{$set: {status: 'Verification Request Declined'}})
        }
        ...
        await db.collection(COL.Requests)
            .findOneAndUpdate({ _id: ObjectIdWithErrorHandler(reqId) },
...
const verifyEmail = async (
    ...
    if (domain.includes('.edu')) {
        ...
        let college = await db.collection(COL.Colleges).findOne(
            { _id: ObjectIdWithErrorHandler(tokenData.collegeId), domains: { $all: [domain] } })
        if (!college) {
            await db.collection(COL.Colleges).updateOne(
